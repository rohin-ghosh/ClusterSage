"""DuckDB storage for ingested and normalized log records."""
...
def initialize_database(connection: duckdb.DuckDBPyConnection) -> None:
    """Create the minimal schema used by the current ingest pipeline."""
    connection.execute(CREATE_LOG_RECORDS_TABLE_SQL)


def insert_log_records(connection: duckdb.DuckDBPyConnection, records: Iterable[LogRecord]) -> int:
    """Persist log records and return the number written."""
    rows = [
        (
            record.id,
            record.timestamp,
